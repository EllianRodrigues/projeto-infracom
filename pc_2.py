...
def receive_messages(socket, address_clockwise, address_anticlockwise):
    ...
    while True:
        ...
        if (decode_message['token']):
            if (decode_message['destination_address'] == 10113):
                ...
                if verify(sem_assinatura.encode(), assinatura, public_key_PC):
                    ...
                    if (roteamento(10113, ('tantofaz', decode_message['source_address'])) == "clockwise"):
...
def send(socket, address_clockwise, address_anticlockwise, message):
    ...
    for destination_address in destination_address_map.values():
            ...
            if (destination_address[1] != 10113):
                if (mandar_primeira_mensagem):                
                    ...
                    if (roteamento(10113, destination_address) == "clockwise"):
                        socket.sendto(datagram.encode(), address_clockwise)
                    else:
                        socket.sendto(datagram.encode(), address_anticlockwise)
                else:
...
clockwise_address = destination_address_map['3']
anticlockwise_address = destination_address_map['1']
...
print("Bem vindo ao PC2")
pedir_input()
# Threads para receber e enviar mensagens
Thread(target=receive_messages, args=(socket_pc2, clockwise_address, anticlockwise_address)).start()
Thread(target=send_message, args=(socket_pc2, clockwise_address, anticlockwise_address)).start()
