        ...
        if (decode_message['token']):
            if (decode_message['destination_address'] == 10117):
                ...
                if verify(sem_assinatura.encode(), assinatura, public_key_PC):
                    ...
                    if (roteamento(10117, ('tantofaz', decode_message['source_address'])) == "clockwise"):
            ...
            if (decode_message['destination_address'] == 10117):
                ...
                if (decode_message['message'] != "Mensagem recebida" and decode_message['message'] != "MENSAGEM VERIFICADA!!! OI, PC10117"):
                    # Envia resposta de volta
    ...
    for destination_address in destination_address_map.values():
            ...
            if (destination_address[1] != 10117):
                if (mandar_primeira_mensagem ):                
                    ...
                    if (roteamento(10117, destination_address) == "clockwise"):
                        socket.sendto(datagram.encode(), address_clockwise)
                    else:
                        socket.sendto(datagram.encode(), address_anticlockwise)
...
socket_pc6.sendto("request_public_key_PC6".encode(), ac_address)
private_key_bytes, _ = socket_pc6.recvfrom(1024)
private_key = rsa.PrivateKey.load_pkcs1(private_key_bytes)

print("Bem vindo ao PC6")
pedir_input()
# Threads para receber e enviar mensagens
Thread(target=receive_messages, args=(socket_pc6, clockwise_address, anticlockwise_address)).start()
Thread(target=send_message, args=(socket_pc6, clockwise_address, anticlockwise_address)).start()
