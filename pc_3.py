...
def receive_messages(socket, address_clockwise, address_anticlockwise):
    ...
    while True:
        ...
        if (decode_message['token']):
            if (decode_message['destination_address'] == 10114):
                ...
                if verify(sem_assinatura.encode(), assinatura, public_key_PC):
                    ...
                    if (roteamento(10114, ('tantofaz', decode_message['source_address'])) == "clockwise"):
            ...
            if (decode_message['destination_address'] == 10114):
                ...
                if (decode_message['message'] != "Mensagem recebida" and decode_message['message'] != "MENSAGEM VERIFICADA!!! OI, PC10114"):
                    # Envia resposta de volta
...
def send(socket, address_clockwise, address_anticlockwise, message):
    ...
    for destination_address in destination_address_map.values():
            ...
            if (destination_address[1] != 10114):
                if (mandar_primeira_mensagem):                
                    ...
                    if (roteamento(10114, destination_address) == "clockwise"):
                        socket.sendto(datagram.encode(), address_clockwise)
                    else:
                        socket.sendto(datagram.encode(), address_anticlockwise)
                else:
...
clockwise_address = destination_address_map['4']
anticlockwise_address = destination_address_map['2']
...
socket_pc3.sendto("request_public_key_PC3".encode(), ac_address)
private_key_bytes, _ = socket_pc3.recvfrom(1024)
private_key = rsa.PrivateKey.load_pkcs1(private_key_bytes)

print("Bem vindo ao PC3")
pedir_input()
# Threads para receber e enviar mensagens
Thread(target=receive_messages, args=(socket_pc3, clockwise_address, anticlockwise_address)).start()
Thread(target=send_message, args=(socket_pc3, clockwise_address, anticlockwise_address)).start()
