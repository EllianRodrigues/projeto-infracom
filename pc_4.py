...
def send(socket, address_clockwise, address_anticlockwise, message):
    ...
    for destination_address in destination_address_map.values():
            ...
            if (destination_address[1] != 10115):
...
clockwise_address = destination_address_map['5']
anticlockwise_address = destination_address_map['3']
...
socket_pc4.sendto("request_public_key_PC4".encode(), ac_address)
private_key_bytes, _ = socket_pc4.recvfrom(1024)
private_key = rsa.PrivateKey.load_pkcs1(private_key_bytes)

print("Bem vindo ao PC4")
pedir_input()
# Threads para receber e enviar mensagens
Thread(target=receive_messages, args=(socket_pc4, clockwise_address, anticlockwise_address)).start()
Thread(target=send_message, args=(socket_pc4, clockwise_address, anticlockwise_address)).start()
